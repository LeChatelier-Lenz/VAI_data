...
# 读取三种出租车的trip_data转化为dataframe
fhv_df = pd.read_parquet("fhv_tripdata_2021-06.parquet")
fhv_df = fhv_df[["dropOff_datetime", "DOlocationID"]]
green_df = pd.read_parquet("green_tripdata_2021-06.parquet")
green_df = green_df[["lpep_dropoff_datetime", "DOLocationID"]]
yellow_df = pd.read_parquet("yellow_tripdata_2021-06.parquet")
yellow_df = yellow_df[["tpep_dropoff_datetime", "DOLocationID"]]
...
fhv_FMR = count_taxi(fhv_df, "dropOff_datetime", "DOlocationID")
green_FMR = count_taxi(green_df, "lpep_dropoff_datetime", "DOLocationID")
yellow_FMR = count_taxi(yellow_df, "tpep_dropoff_datetime", "DOLocationID")

# 将三种出租车的数据合并,注意要考虑到三种出租车的数据长度不一致，需要补零
FMR_list = []
max_len = max(len(fhv_FMR), len(green_FMR), len(yellow_FMR))
for i in range(max_len):
    ...
    FMR_list.append([sum(x) for x in zip(fhv, green, yellow)])

FMR_df = pd.DataFrame(FMR_list, columns=taxi_zones_df['LocationID'])
FMR_df.to_csv("taxi_seq_4h_FMR.csv", index=False)
