...
fhv_df = fhv_df[["pickup_datetime", "PUlocationID"]]
green_df = pd.read_parquet("green_tripdata_2021-06.parquet")
green_df = green_df[["lpep_pickup_datetime", "PULocationID"]]
yellow_df = pd.read_parquet("yellow_tripdata_2021-06.parquet")
yellow_df = yellow_df[["tpep_pickup_datetime", "PULocationID"]]
...
LMR_list = []
max_len = max(len(fhv_LMR), len(green_LMR), len(yellow_LMR))
for i in range(max_len):
    ...
    LMR_list.append([sum(x) for x in zip(fhv, green, yellow)])

LMR_df = pd.DataFrame(LMR_list, columns=taxi_zones_df['LocationID'])
LMR_df.to_csv("taxi_seq_4h_LMR.csv", index=False)
