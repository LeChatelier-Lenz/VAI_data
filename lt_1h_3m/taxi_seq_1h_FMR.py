...
FMR_list = []
max_len = max(len(green_FMR), len(yellow_FMR))
for i in range(max_len):
    ...
    FMR_list.append([sum(x) for x in zip(green, yellow)])

FMR_df = pd.DataFrame(FMR_list, columns=taxi_zones_df['LocationID'])
# FMR_df.to_csv("taxi_seq_1h_FMR.csv", index=False)
FMR_df.to_parquet("taxi_seq_1h_FMR.parquet")
